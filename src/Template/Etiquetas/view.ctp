 ...
 * @var \Ypunto\Admin\Model\Entity\Etiqueta $etiqueta
 */
?>

<div class="ypunto-content etiquetas view">
    <header class="ypunto-content-header">
        <h1 class="ypunto-title"><?= h($etiqueta->nombre) ?></h1>
            ...
            <div class="card ypunto-view-info">
                <div class="card-body strings">
                    <div class="row row-data">
                        ...
                        <div class="col val"><?= h($etiqueta->nombre) ?></div>
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="card ypunto-actions-card">
                <div class="card-body bg-light">
                    <ul class="list-unstyled text-muted small mb-0">
                        <li><i class="fas fa-fw fa-fingerprint"></i> <?= __('Id.') ?> <strong class="text-monospace"><?= $this->Number->format($etiqueta->id) ?></strong></li>
                        <li><i class="far fa-fw fa-calendar-plus"></i> <?= __('Alta') ?> <strong><?= $etiqueta->created ?></strong></li>                        <li><i class="far fa-fw fa-calendar-check"></i> <?= __('Modificado') ?> <strong><?= $etiqueta->modified ?></strong></li>                        <li><i class="fas fa-fw fa-history"></i> <?= __('Historial') ?> <a href="#"><?= __('Versiones anteriores') ?></a></li>
                    </ul>
                </div>
            </div>
        </section>
    ...
    <div class="card related">
            ...
            <ul class="nav nav-pills card-header-pills">
                <li class="nav-item">
                    <a class="nav-link active" data-toggle="tab" href="#entidades" role="tab" aria-controls="entidades" aria-selected="true">
                        <?= __('Entidades') ?> <span class="badge badge-pill badge-success ml-2"><?= count($etiqueta->entidades) ?></span>
                    </a>
                </li>
            </ul>
        </div>
        <div class="tab-content">
            <div class="tab-pane table-responsive fade show active" id="entidades" role="tabpanel" aria-labelledby="profile-tab">
                <?php if (!empty($etiqueta->entidades)): ?>
                <table class="table">
                    <tr>
                    ...
                    <?php foreach ($etiqueta->entidades as $entidades): ?>
                    <tr>
...
<?php $this->append('ypunto-quick-actions') ?>
    ...
    <a class="dropdown-item" href="<?= $this->Url->build(['action' => 'index']) ?>"><?= __('Listado de Etiquetas') ?></a>
